-- SocialPlus: friends list UI improvements and helpers
--
-- This file contains the SocialPlus addon code which augments the Friends/BNET
-- UI (per-row handling, dropdown/right-click menu customizations and helper
-- utilities). The changes in this file intentionally use conservative, best-
...
-- before their definitions resolve to locals and not globals.
local FG_Debug, FG_InitFactionIcon, FG_ApplyGameIcon, FG_GetClientTextureSafe
local FG_GetNumFriends, FG_GetNumOnlineFriends, FG_GetFriendInfoByIndex, FG_GetSelectedFriend, FG_SetFriendNotes
local FG_BNGetNumFriends, FG_BNGetFriendInfo, FG_BNGetFriendInfoByID, FG_BNGetNumFriendInvites, FG_BNGetFriendInviteInfo
local FG_BNGetSelectedFriend, FG_BNGetInfo, FG_BNGetGameAccountInfo, FG_SetBNetFriendNote

-- Forward-declare core update functions to avoid nil globals if called
-- early in initialization flow; we will assign them below.
local SocialPlus_UpdateFriendButton, SocialPlus_UpdateFriends
...
L.INVITE_REASON_OPPOSITE_FACTION = "Ce contact appartient à une faction adverse."
...
L.INVITE_REASON_OPPOSITE_FACTION = "This friend is the opposite faction."
...
--
-- Helper: determine a friend's faction (best-effort) and whether it's opposite our faction.
-- Returns: isOpposite (true/false), reason (string or nil)
...
if not button or not button.travelPassButton then return end
...
local isOpp, reason = SocialPlus_IsOppositeFaction("WOW", id)
...
local isOpp, reason = SocialPlus_IsOppositeFaction("BNET", id)
if isOpp then return false, reason end
...
-- Determine invite eligibility (best-effort) and apply to the row button
local allowed, reason = SocialPlus_GetInviteStatus("WOW", FriendButtons[index].id)
SocialPlus_SetTravelInvite(button, allowed, reason)
end
else
button.background:SetColorTexture(
...
-- Final best-effort check for full invite eligibility (covers opposite faction too)
...
-- Determine whether the currently selected dropdown friend can be invited.
...
-- Best-effort: determine full invite eligibility (this also handles opposite faction)
local allowed, _ = SocialPlus_GetInviteStatus("WOW", id)
if not allowed then
return false
end
